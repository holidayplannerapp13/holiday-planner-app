  ...
  useEffect(() => {
    const holidays = [...calendarificHolidayData, ...culturalHolidayData]
      ...
      .map(h => {
        ...
        return { ...h, country: codeToName.get(key) ?? h.country };
      });
    ...
    grades.forEach(grade => {
      timeframes.forEach(t => {
        ...
        const weeks = generateWeeks(start, end, t.label).map(row => {
          const wkStart = new Date(row._start);
          ...
          <div>
            ...
            {[semester1, semester2, summer].map((tf, idx) => (
              <div key={tf.label}>
                ...
                Start: <input type="date" value={tf.start} onChange={e => {
                  const v = e.target.value;
                  if (idx === 0) setSemester1({ ...tf, start: v });
                  else if (idx === 1) setSemester2({ ...tf, start: v });
                  else setSummer({ ...tf, start: v });
                }} />
                End: <input type="date" value={tf.end} onChange={e => {
                  const v = e.target.value;
                  if (idx === 0) setSemester1({ ...tf, end: v });
                  else if (idx === 1) setSemester2({ ...tf, end: v });
                  else setSummer({ ...tf, end: v });
                }} />
              </div>
            ))}
          </div>
